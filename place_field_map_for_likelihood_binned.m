function L1= place_field_map_for_likelihood_binned(x_,y_,sigma,As,Omegas,wrkrs)
...
for key_pt=1:numel(x_)
    ...
    % bin the mu_x and mu_y to the closest bins in As and Omegas

    [~,idx_binX]=(min(abs(mux-As)));
    [~,idx_binY]=(min(abs(muy-Omegas)));
    
    mux_b=As(idx_binX);
    muy_b=Omegas(idx_binY);

    % temp place field like map from this keypoint   
    temp=(1/(2*pi*(sigma^2))).* exp(-(...
        ( ( (X-mux_b)./rgx ).^2 + ( (Y-muy_b)./rgy ).^2  )./(2*(sigma^2)) ));
