...
for k=1:dd
    ...
    target_num= set_active_target(k,env.blocks);
...
%% caching logic, based on posterior and reward rate signal entropies
   ...
   if(~cache_flag)
     ...
     if(anchors_no(k)>1)
       ...
       %% (1) resample actions from the reset prior and connect them.
       [mu_anchors,omega_anchors,anchors_no(k)]= Sampling_next_actions(prior,sampler,initial_ancs,As,Os,merging_criterion,r_bounds,c_bounds);
...
%      %% add noise to both sampled controls parameters:
     ...
     if(anchors_no(k)>1)
       ...
       %% (2)simulate this new run
     [target_hit(k),hit_time(k)]=simulate_a_run(pos_x,pos_y,target_num,env,hit_time(k));
       %% (3)update your estimate now
     [posterior]=Bayes_update_for_actions_params(target_hit(k),mus_,omegas_,sigma_ridge,As,Os,prior,wrkrs);  
       %% (4) resample the new posterior and connect actions for the next run after caching
      [mu_anchors,omega_anchors,anchors_no(k)]= Sampling_next_actions(posterior,sampler,initial_ancs,As,Os,merging_criterion,r_bounds,c_bounds);
      [~,min_speed_anchor]=min(mu_anchors);
      initial_hd=omega_anchors(min_speed_anchor);
