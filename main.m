...
spc=1000000;
[prior_global,r_bounds,c_bounds]= set_control_actions_space(Rs,Os,env.arena_dimensions,spc);
prior_global=boolean(prior_global);
%home radius to exclude
[prior_exc_home,r_home,c_home]=semi_circle_around_home(prior_global,min_radius_around_home,Rs,Os);
%home radius to exclude
...
for k=1:dd
     ...
     if(k==1) 
       ...
       [Gsol, ~, ~]=connect_anchors_tsp([ 0 r_anchors]',[0  omega_anchors]',initial_ancs+1,Rs,Os);
       [r_anchors,omega_anchors]=reorder_actions_anchors([0 r_anchors],[ 0 omega_anchors],Gsol);
       [rs_,omegas_,pos_x,pos_y]= trajectory_planner(r_anchors,omega_anchors,env,clearnce,Os,Rs,ka,w2_L,tol_vec(k));
...
%          posterior=new_posterior;
      ...
      %% sampler function for the next actions calling either: proportional or peak sampler
         [r_anchors,omega_anchors,anchors_no(k+1)]= Sampling_next_actions(posterior,sampler,initial_ancs,Rs,Os,merging_criterion,r_bounds,c_bounds,...
             Rs(min(c_home)),percentile_peak_sampler,...
             bestFitDataOffset,bestFitMeanToScaleRatio,bestNormalFitAngleScale);

      if(anchors_no(k+1)>1)
         
         [Gsol,~,~]=connect_anchors_tsp([ 0 r_anchors]',[ 0 omega_anchors]',anchors_no(k+1)+1,Rs,Os);       
         [r_anchors,omega_anchors]=reorder_actions_anchors([0 r_anchors],[ 0 omega_anchors],Gsol);
         [rs_new,omegas_new,pos_xnew,pos_ynew]= trajectory_planner(r_anchors,omega_anchors,env,clearnce,Os,Rs,ka,w2_L,tol_vec(k+1));

      
      else
      
         r_anchors=[0, r_anchors, 0];
         omega_anchors=[0, omega_anchors, 0];
         [rs_new,omegas_new,pos_xnew,pos_ynew]= trajectory_planner(r_anchors,omega_anchors,env,clearnce,Os,Rs,ka,w2_L,tol_vec(k+1));
       ...
       %% sample and plan the new route from the current active posterior.
         [r_anchors,omega_anchors,anchors_no(k+1)]= Sampling_next_actions(active_prior,sampler,initial_ancs,Rs,Os,merging_criterion,r_bounds,c_bounds,...
             Rs(min(c_home)),percentile_peak_sampler,...
             bestFitDataOffset,bestFitMeanToScaleRatio,bestNormalFitAngleScale);
