...
% Total number of parameters to optimize:
    ...
    % - (1): offset angles for heading vectors between anchor pairs.
...
lb(2:2+(no_anchors-1))=repmat(r_home,1,no_anchors); % Radii >= r_home.
lb(2+no_anchors-1)=0; % last anchor radius (Home) = 0.
lb(2)=0; % First anchor radius (Home)= 0.
...
ub(2:2+(no_anchors-1))=repmat(rg_r,1,no_anchors);  % Radii <= rg_r.
...
for n=1:no_anchors
    ...
    r_n=p(1+n); % Current anchor's radius.
