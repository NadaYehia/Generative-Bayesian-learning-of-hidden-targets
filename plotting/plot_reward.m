...
scale = 2;
trials=size(corr_pop,2);
smooth_corr_pop=movmean(corr_pop,5,2);


% sem_acc= scale.*(std(smooth_corr_pop_avg(:,1:trials))./sqrt(size(smooth_corr_pop_avg,1)));

std_acc=std(smooth_corr_pop(:,1:trials));
...
fontname(f1,"Arial");
hold on, shadedErrorBar_std_mean(1:trials, mean(smooth_corr_pop(:,1:trials),1) ,std_acc ,{'color',[0.6 0.5 0.5]}); hold on;
% plot_shaded_error_bar(smooth_corr_pop,[0.5 0.6 0.5])
ylim([0 1]);
ylabel('reward rate');
